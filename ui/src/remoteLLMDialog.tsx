...
import { Button, Callout, Dialog, SecretField, TextField } from "./ui";

export function RemoteLlmApiKeyDialog({ label, configured, destination, destinationEditable, approvedEndpoint, close, onSave, onDelete }: { label: string; configured: boolean; destination: string; destinationEditable: boolean; approvedEndpoint: string | null; close: () => void; onSave: (key: string, destination: string) => Promise<string | void>; onDelete: () => Promise<string | void> }) {
    ...
    return <Dialog open title={`${label} API Key`} description="Credentials are stored by the local BV Nodepack backend and are never written into the workflow." size="small" onClose={close} footer={<><Button intent="danger" disabled={!configured || busy} onClick={() => run(onDelete)}>Delete key</Button><Button intent="ghost" disabled={busy} onClick={close}>Cancel</Button><Button intent="primary" loading={busy} disabled={!key.trim() || !target.trim()} onClick={() => run(() => onSave(key, target.trim()))}>Save key</Button></>}>
        ...
        {destinationEditable
            ? <TextField label="Destination (Chat Completions URL)" value={target} spellCheck={false} onValue={setTarget} help="HTTPS or loopback HTTP. The workflow cannot change this address; only saving the key here can."/>
