...
const btn = (label: string, action: () => void, variant: "primary" | "secondary" | "ghost" | "danger" = "secondary") => { const value = createBvButton(label, variant); value.onclick = action; return value; };
...
export const openDetectorRegistryDialog = async (api: any, stored: unknown, save: (value: string) => void) => {
    ...
    const shell = createBvDialog({ title: "Detector Registry", description: "Configure the detector models available to regional detailer plans. Models are loaded through the installed Impact providers.", size: "large" });
    const dialog = shell.dialog;
    const list = el("div");
    list.className = "bv-ui-card-list";
    const field = (caption: string, control: HTMLElement) => createBvField(caption, control);
    const render = () => {
        ...
        config.detectors.forEach((entry, index) => {
            ...
            card.append(field("Detector ID", id), field("Provider", provider), field("Model", model), field("SAM model (optional)", sam), field("SAM device", mode), btn("Remove", () => { config.detectors.splice(index, 1); render(); }, "danger"));
            list.append(card);
        });
        if (!config.detectors.length) list.append(el("p", "No detectors configured yet."));
    };
    shell.footer.append(btn("Restore empty registry", () => { config = emptyDetectorRegistryConfig(); render(); }, "ghost"), btn("Add detector", () => {
    ...
    }), btn("Cancel", () => dialog.close(), "ghost"), btn("Save registry", () => {
