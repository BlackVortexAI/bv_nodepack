...
import{applyInteractiveFanIn,removeInteractiveFanIn}from"./interactiveFanIn.js";
import{installProjectedPortInteraction}from"./projectedPortInteraction.js";
...
function applyProjectedWidgetStart(node:any,plan:ReturnType<typeof resolveNodePresentation>){
    ...
    const hiddenVertical=entries.some(({projected,slot})=>slot&&!(projected.direction==="input"&&widgetBacked(node,slot))&&!projected.visible);
    const visible=entries.filter(entry=>rowEligible(entry)&&entry.projected.visible);
...
export function removeNodePresentation(node:any){cancelScheduledProjectedPortLayout(node);restoreProjectedWidgetStart(node);removeProjectedProviderAnchors(node);removeInteractiveFanIn(node);removePresentationSizeLifecycle(node);removeNodes2NodePresentation(node)}
...
export function applyClassicNodePresentation(node:any,nodeType:string,options:ClassicPresentationOptions={}){
    ...
    if(node.__bvPresentationHasLegacy)installProjectedPortInteraction(node);
    ...
    const reservedInputs=plan.ports.filter(port=>port.direction==="input"&&port.visible).length;
    const reservedOutputs=plan.ports.filter(port=>port.direction==="output"&&port.visible).length;
