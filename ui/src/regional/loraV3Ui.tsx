...
import { compactLoraConsumerNode, connectLoraConsumerTreeCollectors, ensureLoraCollectorOutput, ensureLoraConsumerInputs, linkedLocalLoraCollectors, localLoraCollectors, migrateLegacyLoraCollectorLink, scheduleCompactLoraConsumerNode, upstreamLoraTransformer } from "./loraV3Graph";
...
export function installLoraV3ConsumerSlot(node:any){ensureCanvasVisibility();for(const index of ensureLoraConsumerInputs(node)){const input=node.inputs?.[index];if(input){input.hidden=true;input.label="";input.__bvM0VisualHidden=true;input.__bvM0PortHidden=true;input.__bvM0ResourceSlot=true;}}migrateLegacyLoraCollectorLink(node);const transformer=upstreamLoraTransformer(node);if(transformer){const collectors=linkedLocalLoraCollectors(transformer).filter(Boolean);if(collectors.length)connectLoraConsumerTreeCollectors(transformer,collectors);}scheduleCompactLoraConsumerNode(node);}
