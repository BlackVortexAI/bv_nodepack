...
type ScopeCallbacks={onCollector:(entryId:string,collectorId:string)=>void;onResource:(entryId:string,resourceId:string)=>void;onAdd:(target:LoraV3Target)=>void;onRemove:(entryId:string,target:LoraV3Target)=>void};
...
export function LoraV3ScopePicker({collectors,config,target,resolved,onCollector,onResource,onAdd,onRemove}:ScopeProps){
    ...
    return <div className="bv-lora-v3-scope-picker">{entries.map((entry,index)=><section className="bv-lora-v3-stack-row" key={entry.id}><strong>LoRA Stack {index+1}</strong><ResourcePicker collectors={collectors} collectorId={entry.source.collector_id} resourceId={entry.source.resource_id} resolved={typeof resolved==="function"?resolved(entry):resolved} onCollector={id=>onCollector(entry.id,id)} onResource={id=>onResource(entry.id,id)}/><Button density="compact" intent="ghost" iconOnly aria-label={`Remove LoRA Stack ${index+1}`} title="Remove LoRA stack" onClick={()=>onRemove(entry.id,target)}><svg className="bv-trash-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M4 7h16M9 7V4h6v3M7 7l1 13h8l1-13M10 11v5M14 11v5"/></svg></Button></section>)}<Button intent="secondary" disabled={!collectors.some(collector=>collector.resources.length>0)} onClick={()=>onAdd(target)}>Add LoRA stack</Button></div>;
}
...
export function LoraV3ResourcePickerPanel({collectors,config,resolved,targetOptions=[],onCollector,onResource,onAddExternal,onRemove}:{collectors:ResourcePickerCollector[];config:LoraV3Config;resolved:boolean|((entry:LoraV3Entry)=>boolean);targetOptions?:LoraV3TargetOption[];onCollector:(entryId:string,collectorId:string)=>void;onResource:(entryId:string,resourceId:string)=>void;onAddExternal:(target:LoraV3Target)=>void;onRemove:(entryId:string,target:LoraV3Target)=>void}){
    return <div className="bv-lora-v3-picker-panel">{targetOptions.map(option=><section className="bv-lora-v3-target" key={option.value}><h3>{option.label}</h3><LoraV3ScopePicker collectors={collectors} config={config} target={option.target} resolved={resolved} onCollector={onCollector} onResource={onResource} onAdd={onAddExternal} onRemove={onRemove}/></section>)}</div>;
}
