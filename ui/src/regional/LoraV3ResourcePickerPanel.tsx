...
export function withoutLoraV3Target(config:LoraV3Config,target:LoraV3Target,entryId?:string){const entries=config.entries.flatMap(entry=>{if(entryId&&entry.id!==entryId)return [entry];const targets=entry.targets.filter(candidate=>!sameLoraTarget(candidate,target));return targets.length?[{...entry,targets}]:[];});return {...config,collector_id:entries.some(entry=>entry.source.kind==="external")?config.collector_id:null,entries};}

type ScopeCallbacks={onCollector:(id:string)=>void;onResource:(entryId:string,resourceId:string)=>void;onAdd:(target:LoraV3Target)=>void;onRemove:(entryId:string,target:LoraV3Target)=>void};
type ScopeProps=ScopeCallbacks&{collectors:ResourcePickerCollector[];config:LoraV3Config;target:LoraV3Target;resolved:boolean};
