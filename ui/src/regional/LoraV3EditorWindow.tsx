...
import { loraV3Catalog, loraV3Resolved, readNodeLoraV3Config, writeNodeLoraV3Config } from "./loraV3Ui";
...
export default function LoraV3EditorWindow({open,node,onClose}:{open:boolean;node:any|null;onClose:()=>void}){
    ...
    if(!node)return null;
    const collectors=loraV3Catalog(node).filter(item=>item.resources.length>0),resolved=loraV3Resolved(node,config);
