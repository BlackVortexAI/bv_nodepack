...
import{LORA_V3_INVENTORY_CHANGED_EVENT,installLoraRegistryInventorySource,storeLoraRegistryInventory}from"./loraV3Inventory";
...
export function installLoraRegistryUi(nodeType:any,nodeData:any,api:any,graphOwner:(node:any)=>any=(node:any)=>node?.graph){
    ...
    let spec:any;
    const bindRefresh=(node:any)=>{
        if(node.__bvGlobalRegistryRefresh||typeof window==="undefined")return;
        const listener=(event:any)=>{if(String(event.detail?.node?.type??event.detail?.node?.comfyClass)==="BV LoRA Registry")refreshReactNodeWidget(node,nodeData.name,spec)};
        node.__bvGlobalRegistryRefresh=listener;window.addEventListener(LORA_V3_INVENTORY_CHANGED_EVENT,listener);
    };
    const removed=nodeType.prototype.onRemoved;nodeType.prototype.onRemoved=function(){if(this.__bvGlobalRegistryRefresh){window.removeEventListener(LORA_V3_INVENTORY_CHANGED_EVENT,this.__bvGlobalRegistryRefresh);delete this.__bvGlobalRegistryRefresh;}return removed?.apply(this,arguments)};
    const state=(node:any)=>{bindLoraV3Graph(node,graphOwner(node));const hidden=configWidget(node);if(!hidden)return null;const store=(value:string)=>storeLoraRegistryInventory(node,hidden,value,()=>refreshReactNodeWidget(node,nodeData.name,spec));return{hidden,store}},open=(node:any)=>{const current=state(node);if(current)openLoraRegistryDialog(api,current.hidden.value,current.store,`lora-registry:${String(node.id)}`,()=>configWidget(node)?.value)},openLibrary=(node:any,stackId:string)=>{const current=state(node);if(current)openLoraCatalogLibraryDialog(api,()=>configWidget(node)?.value,stackId,current.store,`lora-library:${String(node.id)}`)};
    spec={id:SPEC_ID,name:"lora_registry_widget",minHeight:72,maxHeight:340,growWithContent:true,overflow:"auto",nativeActions:[{id:"open-registry",name:"open_lora_registry",label:"Open LoRA Registry",invoke:open}],render:(node:any)=>{
        const hidden=configWidget(node);if(!hidden)return null;bindRefresh(node);
        ...
        return createElement("div",{className:"bv-ui-stack"},workflowGlobalLoraSelection(node).conflict?createElement(Callout,{tone:"warning",title:"Multiple active Global Registries",children:"Turn the intended Registry's Global switch off and on to select it. Other Global Registries will be disabled."}):null,createElement(LoraRegistryNodeWidget,{stored:hidden.value,onStored:current.store,onOpenLibrary:(stackId:string)=>openLibrary(node,stackId)}));
    }};
