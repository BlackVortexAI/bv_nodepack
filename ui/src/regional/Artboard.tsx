...
export default function Artboard(props: Props) {
    ...
    const geometriesFor = (region: Region) => {
        ...
        if (props.isolate && selectedLayerId) geometries = geometries.filter(item => geometryLayerId(item) === selectedLayerId);
        return geometries;
    };
    ...
    >
        ...
        {props.cursor && props.tool.startsWith("brush") && <div className={`brush-cursor ${props.brush.shape}`} style={{ left: `${props.cursor.x * 100}%`, top: `${props.cursor.y * 100}%`, width: `${cursorWidth}%`, aspectRatio: "1" }}><i style={{ width: `${props.brush.hardness * 100}%`, height: `${props.brush.hardness * 100}%` }}/></div>}
        </div></div>
