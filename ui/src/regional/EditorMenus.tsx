...
export default function EditorMenus(props: Props) {
    ...
    const menu = props.openMenu,{menuOverflow:showMenusInOverflow}=useBvWindowHeaderLayout();
    ...
    return <div className="editor-menus">{showMenusInOverflow?<Popover trigger={({toggle})=><Button intent="ghost" density="compact" aria-label="Editor menus" onClick={toggle}>•••</Button>}><div className="editor-menus-overflow-panel">{buttons}</div></Popover>:buttons}
