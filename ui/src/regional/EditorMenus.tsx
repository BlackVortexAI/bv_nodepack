...
export default function EditorMenus(props: Props) {
    ...
    return <div className="editor-menus" ref={root}>
        ...
        <div className="menu-item"><button onClick={() => toggle("view")}>View</button>{menu === "view" && <div className="menu-popover view-menu"><label>Mask Display <output>{Math.round(props.displayOpacity * 100)} %</output><input type="range" min=".05" max="1" step=".05" value={props.displayOpacity} onChange={event => props.onDisplayOpacity(+event.target.value)}/></label><label>Background Image <output>{Math.round(props.backgroundOpacity * 100)} %</output><input type="range" min="0" max="1" step=".05" value={props.backgroundOpacity} onChange={event => props.onBackgroundOpacity(+event.target.value)}/></label><button className={props.binaryMaskPreview ? "active" : ""} onClick={props.onToggleBinaryMaskPreview}>{props.binaryMaskPreview ? "✓ " : ""}Binary Mask Preview</button><button disabled={!props.hasSelection} className={props.isolate ? "active" : ""} onClick={props.onToggleIsolate}>Isolate Selection</button><small>Affects the editor display only.</small></div>}</div>
