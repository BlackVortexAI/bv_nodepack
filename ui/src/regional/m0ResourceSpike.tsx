...
function renderMultiPicker(node:any){
    ...
    if(!host){host=document.createElement("div");host.className="bv-ui bv-m0-picker-host";host.style.position="relative";host.style.zIndex="1";node.__bvM0PickerHost=host;const dom=node.addDOMWidget?.("bv_multi_resource_picker","div",host,{serialize:false});if(dom)dom.serialize=false;}
    ...
    draw();setMultiDebug(node,Boolean(node.properties?.bvM0DebugVisible),false);
}
