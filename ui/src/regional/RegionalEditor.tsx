...
export default function RegionalEditor({ open, activationToken=0, nodes, initialNode, backgrounds, loraStacks, onClose }: Props) {
    ...
    useEffect(() => {
        ...
        try {
            ...
            setViewState(stored); setDocumentValue(next); setPrimitiveDraft(regionalEditorDraft(node)); setLoraBindings(nextBindings); setLoraV3Config(readNodeLoraV3Config(node)); setDetailerEasyConfig(readDetailerEasyConfig(node,next.regions)); setSelectedRegionId(regionId); setSelectedLayerId(layerId); setSelectedLayerIds(layerId ? [layerId] : []); setSelectionAnchorId(layerId); setTool(stored.tool); setBrush(stored.brush); setDisplayOpacity(stored.displayOpacity); setBackgroundOpacity(stored.backgroundOpacity); setIsolate(stored.isolate && !!layerId); setHistory([]); setFuture([]); setError("");
        }
            ...
            <main className="bv-regional-dock-host">
              <BvDockLayout storageId={`regional-editor:${documentValue.document_id}`} resetSignal={dockResetSignal} defaultModel={{ global:{ tabEnableClose:false, tabEnablePopout:false, tabEnablePopoutIcon:false, tabEnablePopoutFloatIcon:true, tabSetMinWidth:180, tabSetMinHeight:120 }, borders:[], layout:{ type:"row", children:[
                ...
                { type:"row", weight:24, minWidth:260, children:[
                    { type:"tabset", weight:50, children:[{ type:"tab", id:"bv.regional.global", name:"Global", component:"global", enablePopoutFloatIcon:true }] },
                    { type:"tabset", weight:25, children:[{ type:"tab", id:"bv.regional.region", name:"Region", component:"region", enablePopoutFloatIcon:true }] },
                    { type:"tabset", weight:25, children:[{ type:"tab", id:"bv.regional.document", name:"Document", component:"document", enablePopoutFloatIcon:true }] }
                ]}
              ]} }} panels={[
