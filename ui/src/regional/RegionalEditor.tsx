...
import { clone, Geometry, geometryAuthoring, geometryLayerId, geometryLayers, newRegion, parseDocument, Point, preserveDocumentIdentity, RegionalDocument, Region, uuid } from "./model";
...
export default function RegionalEditor({ open, nodes, initialNode, backgrounds, onClose }: Props) {
    ...
    useEffect(() => {
        ...
        try {
            ...
            setViewState(stored); setDocumentValue(next); setSelectedRegionId(regionId); setSelectedLayerId(layerId); setTool(stored.tool); setBrush(stored.brush); setDisplayOpacity(stored.displayOpacity); setBackgroundOpacity(stored.backgroundOpacity); setIsolate(stored.isolate && !!layerId); setHistory([]); setFuture([]); setError("");
        }
