...
export default function RegionalEditor({ open, activationToken=0, nodes, initialNode, backgrounds, loraStacks, onClose }: Props) {
    ...
    useEffect(() => {
        ...
        try {
            ...
            setViewState(stored); setDocumentValue(next); setLoraBindings(nextBindings); setLoraV3Config(readNodeLoraV3Config(node)); setDetailerEasyConfig(readDetailerEasyConfig(node,next.regions)); setSelectedRegionId(regionId); setSelectedLayerId(layerId); setSelectedLayerIds(layerId ? [layerId] : []); setSelectionAnchorId(layerId); setTool(stored.tool); setBrush(stored.brush); setDisplayOpacity(stored.displayOpacity); setBackgroundOpacity(stored.backgroundOpacity); setIsolate(stored.isolate && !!layerId); setHistory([]); setFuture([]); setError("");
        }
