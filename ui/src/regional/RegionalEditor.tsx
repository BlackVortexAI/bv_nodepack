...
import {useImageDimensions} from "../ui/useImageDimensions";
import {regionalActiveTools} from "./regionalToolState";
import {CONNECTED_IMAGE_SELECTION} from "./inputImagePreview";
import {useInputImagePreview} from "./useInputImagePreview";
...
import { RegionLayerTree, TextField, useBvConfirm } from "../ui/components";
...
import { applyRegionalCanvasSize, applyRegionalPrimitiveDraft, persistRegionalEditorDraft, regionalEditorDraft, type RegionalEditorDraft } from "./milestoneE";
...
export default function RegionalEditor({ open, activationToken=0, activityScope, nodes, initialNode, canvasCatalog, canvasApiUrl, loraStacks, onClose }: Props) {
    ...
    const [lutEasyConfig,setLutEasyConfig]=useState<LutEasyConfig>(()=>emptyLutEasyConfig());
    const [primitiveDraft,setPrimitiveDraft]=useState<RegionalEditorDraft|null>(()=>initialNode?regionalEditorDraft(initialNode):null);
    ...
    useEffect(()=>{if(!open||!node)return;const refresh=()=>setLutEasyConfig(readLutEasyConfig(node));window.addEventListener("bv-regional-lut-config-changed",refresh);return()=>window.removeEventListener("bv-regional-lut-config-changed",refresh);},[node,open]);
    useEffect(() => {
        if (!open || !node) return;
        try {
            ...
            setViewState(stored); setDocumentValue(next); setCanvasSelection(regionalCanvasSelectionsForScope(canvasSelections.current,activityScope).get(next.document_id)??CONNECTED_IMAGE_SELECTION); setPrimitiveDraft(regionalEditorDraft(node)); setLoraBindings(nextBindings); setLoraV3Config(readNodeLoraV3Config(node)); setDetailerEasyConfig(readDetailerEasyConfig(node,next.regions)); setLutEasyConfig(readLutEasyConfig(node)); setSelectedRegionId(regionId); setSelectedLayerId(layerId); setSelectedLayerIds(layerId ? [layerId] : []); setSelectionAnchorId(layerId); setTool(stored.tool); setBrush(stored.brush); setDisplayOpacity(stored.displayOpacity); setBackgroundOpacity(stored.backgroundOpacity); setIsolate(stored.isolate && !!layerId); setHistory([]); setFuture([]); setError("");
        }
    ...
    useEffect(()=>{if(!node||!documentValue)return;const next=reconcileDetailerEasyConfig(detailerEasyConfig,documentValue.regions);if(JSON.stringify(next)!==JSON.stringify(detailerEasyConfig))setDetailerEasyConfig(writeDetailerEasyConfig(node,next));},[node,documentValue?.regions,detailerEasyConfig]);
    ...
    const updatePrimitiveDraft=(field:string,value:string)=>{if(!node||!documentValue)return;const raw={title:documentValue.title,"canvas.width":String(documentValue.canvas.width),"canvas.height":String(documentValue.canvas.height),...(primitiveDraft?.raw??{}),[field]:value},next=applyRegionalPrimitiveDraft(documentValue,raw);setPrimitiveDraft(next.draft);persistRegionalEditorDraft(node,next.draft);persist(next.canonical)};
    ...
    // BV-LEGACY(marked=2026-08-25, remove-after=2026-10-25): Regions export v1 import.
    // Remove v1 acceptance after exported region bundles have passed the migration window.
    ...
    const inputPreview=useInputImagePreview(node,"canvas_image",canvasApiUrl,open);
    ...
    const canvasBackground=canvasSelection===CONNECTED_IMAGE_SELECTION?inputPreview?.src:regionalCanvasImageUrl(resolvedCanvasImage,canvasApiUrl);
    const imageDimensions=useImageDimensions(canvasBackground,node,open);
    const canMatchImage=!!imageDimensions&&[imageDimensions.width,imageDimensions.height].every(value=>Number.isInteger(value)&&value>=64&&value<=16384);
    const matchImageSize=()=>{if(!canMatchImage||!imageDimensions||!node||!documentValue)return;cancelGesture();const next=applyRegionalCanvasSize(documentValue,imageDimensions,primitiveDraft);setPrimitiveDraft(next.draft);persistRegionalEditorDraft(node,next.draft);persist(next.canonical)};
    if (!open) return null;
            ...
            <main className="bv-regional-dock-host">
              <BvDockLayout storageId={`regional-editor:${documentValue.document_id}`} resetSignal={dockResetSignal} defaultModel={{ global:{ tabEnableClose:false, tabEnablePopout:false, tabEnablePopoutIcon:false, tabEnablePopoutFloatIcon:true, tabSetMinWidth:180, tabSetMinHeight:120 }, borders:[], layout:{ type:"row", children:[
                { type:"row", weight:22, minWidth:220, children:[
                    { type:"tabset", weight:55, children:[{ type:"tab", id:"bv.regional.regions", name:"Regions", component:"regions", enablePopoutFloatIcon:true }] },
                    { type:"tabset", weight:45, children:[{ type:"tab", id:"bv.regional.selection", name:"Selection", component:"selection", enablePopoutFloatIcon:true }] }
                ]},
                ...
                <Artboard key={documentValue.document_id} document={documentValue} background={canvasBackground} footerControls={<><RegionalCanvasImageSelect input={inputPreview} images={canvasImages} selection={canvasSelection} onSelection={selectCanvasImage} apiURL={canvasApiUrl}/><Button disabled={!canMatchImage} title={canMatchImage?`Set canvas to ${imageDimensions!.width} × ${imageDimensions!.height}`:"Select an available image with dimensions from 64 to 16384 pixels"} onClick={matchImageSize}>Match image size</Button></>} selectedRegionId={selectedRegionId} selectedLayerId={selectedLayerId} draft={draft} selectionBounds={selectionBounds} cursor={cursor} tool={tool} brush={brush} canSubtract={!!selectedLayer && !selectedLayer.authoring.locked} displayOpacity={displayOpacity} backgroundOpacity={backgroundOpacity} isolate={isolate} binaryMaskPreview={viewState.binaryMaskPreview} initialView={viewState.artboard} onView={updateArtboardView} onTool={next => { setTool(next); cancelGesture(); }} onBrush={setBrush} onPointerDown={pointerDown} onPointerMove={pointerMove} onPointerUp={pointerUp} onPointerCancel={cancelGesture} onPointerLeave={() => { if (!gesture.current) setCursor(null); }} onDoubleClick={doubleClick}/>
                ...
                , { id:"document", layoutId:"bv.regional.document", title:"Document", weight:12, minWidth:240, content:documentSettings }
              ]}/>
            </main>
        </>}
