...
export type DetailerPlanJob = { region_ids: string[]; primary_region_id: string; mask_composition: "union" | "intersection" | "subtract"; prompt_composition: "primary" | "context"; detector_id?: string; conditioning: DetailerConditioning; detector: DetailerDetection };
export type DetailerPlanConfig = { schema: "bv.detailer_plan"; version: 1; jobs: DetailerPlanJob[] };
...
export const defaultDetailerPlan = (regions: DetailerPlanRegion[]): DetailerPlanConfig => ({ schema: "bv.detailer_plan", version: 1, jobs: eligibleDetailerRegions(regions).map(region => ({ region_ids: [region.id], primary_region_id: region.id, mask_composition: "union", prompt_composition: "context", conditioning: defaultConditioning(), detector: defaultDetection() })) });
export const parseDetailerPlanConfig = (value: unknown, regions: DetailerPlanRegion[]): DetailerPlanConfig => {
    ...
    const jobs = Array.isArray(parsed?.jobs) ? parsed.jobs.flatMap((job: any) => {
        ...
        const conditioning = job.conditioning ?? {}, detector = job.detector ?? {};
        return [{ region_ids: ids, primary_region_id: ids.includes(String(job.primary_region_id)) ? String(job.primary_region_id) : ids[0], mask_composition: ["union", "intersection", "subtract"].includes(job.mask_composition) ? job.mask_composition : "union", prompt_composition: job.prompt_composition === "primary" ? "primary" : "context", ...(String(job.detector_id ?? "").trim() ? { detector_id: String(job.detector_id).trim() } : {}),
            ...
            detector: { roi_padding: finite(detector.roi_padding, 0.15, 0, 2), threshold: finite(detector.threshold, 0.5, 0, 1), dilation: Math.round(finite(detector.dilation, 0, -512, 512)), crop_factor: finite(detector.crop_factor, 1.5, 1, 20), drop_size: Math.round(finite(detector.drop_size, 10, 1, 8192)), ...(String(detector.query ?? "").trim() ? { query: String(detector.query).trim() } : {}), ...(Array.isArray(detector.labels) ? { labels: detector.labels.map(String).map((item: string) => item.trim()).filter(Boolean) } : {}) },
        } as DetailerPlanJob];
    }) : [];
    return { schema: "bv.detailer_plan", version: 1, jobs };
};
