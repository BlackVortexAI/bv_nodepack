...
export function boundsOf(geometry: Geometry, canvas: CanvasSize = { width: 1, height: 1 }): Bounds {
    if (geometry.type === "rect" || geometry.type === "ellipse" || geometry.type === "raster_mask") return { x: geometry.x, y: geometry.y, width: geometry.width, height: geometry.height };
    const xs = geometry.points.map(point => point.x), ys = geometry.points.map(point => point.y);
    if (geometry.type === "polygon") return { x: Math.min(...xs), y: Math.min(...ys), width: Math.max(...xs) - Math.min(...xs), height: Math.max(...ys) - Math.min(...ys) };
...
export function hitTest(geometry: Geometry, point: Point, canvas: CanvasSize = { width: 1, height: 1 }) {
    ...
    if (geometry.type === "ellipse") { const rx = geometry.width / 2, ry = geometry.height / 2; return ((point.x - geometry.x - rx) / rx) ** 2 + ((point.y - geometry.y - ry) / ry) ** 2 <= 1; }
    if (geometry.type === "polygon") { let inside = false; for (let i = 0, j = geometry.points.length - 1; i < geometry.points.length; j = i++) { const a = geometry.points[i], b = geometry.points[j]; if ((a.y > point.y) !== (b.y > point.y) && point.x < (b.x - a.x) * (point.y - a.y) / (b.y - a.y) + a.x) inside = !inside; } return inside; }
...
export function moveGeometry(source: Geometry, dx: number, dy: number, canvas?: CanvasSize): Geometry {
    ...
    if (geometry.type === "rect" || geometry.type === "ellipse" || geometry.type === "raster_mask") { geometry.x += dx; geometry.y += dy; }
...
export function setBounds(source: Geometry, target: Bounds, canvas?: CanvasSize): Geometry {
    ...
    if (geometry.type === "rect" || geometry.type === "ellipse" || geometry.type === "raster_mask") return { ...geometry, ...target };
    ...
    if (geometry.type === "brush_stroke") geometry.size = clamp(geometry.size * Math.sqrt(sx * sy), .001, 1);
    return geometry;
}
...
export function moveLayer(geometries: Geometry[], dx: number, dy: number, canvas?: CanvasSize): Geometry[] {
    ...
    return geometries.map(geometry => {
        ...
        if (result.type === "rect" || result.type === "ellipse" || result.type === "raster_mask") { result.x += dx; result.y += dy; }
...
export function setLayerBounds(geometries: Geometry[], target: Bounds, canvas?: CanvasSize): Geometry[] {
    ...
    return geometries.map(source => {
        ...
        if (geometry.type === "rect" || geometry.type === "ellipse" || geometry.type === "raster_mask") {
