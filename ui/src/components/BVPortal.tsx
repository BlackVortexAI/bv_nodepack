...
import { BvDockLayout, BvManagedWindow, Callout, CheckboxField, ResetLayoutButton } from "../ui";
...
function ControlCenterLiveView(){
    ...
    return <div className="bv-control-live-list">{config.controls.map(control=><CheckboxField key={control.id} label={control.name} checked={control.enabled} onValue={value=>toggle(control.id,value)} />)}</div>;
}
...
const BvPortal: FC<IBVPortalProps> = ({open,onClose,hasControlNodes=true}) => {
    ...
    return <BvManagedWindow open={open} title="BV Control Center" shortTitle="Control Center" initialGeometry={{width:860,height:640}} minSize={{width:420,height:360}} onClose={onClose} status="Workflow-wide controls are saved automatically." actions={<ResetLayoutButton storageId="control-center" editorType="control-center" onClick={()=>setLayoutReset(value=>value+1)}/>}>
