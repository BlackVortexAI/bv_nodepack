...
function notifySessionLayoutDraft(key:string){sessionRevisions.set(key,(sessionRevisions.get(key)??0)+1);for(const subscriber of sessionSubscribers.get(key)??[])subscriber()}
export function getSessionLayoutRevision(key:string){return sessionRevisions.get(key)??0}
export function subscribeSessionLayoutDraft(key:string,subscriber:()=>void){const subscribers=sessionSubscribers.get(key)??new Set<()=>void>();subscribers.add(subscriber);sessionSubscribers.set(key,subscribers);return()=>{subscribers.delete(subscriber);if(!subscribers.size)sessionSubscribers.delete(key)}}
export function setSessionLayoutDraft(key:string,layout:unknown,status:BvLayoutStatus,profileId?:string,notify=true){sessionDrafts.set(key,{layout:structuredClone(layout),status,profileId});notifySessionLayoutDraft(key);if(notify)window.dispatchEvent(new CustomEvent(BV_LAYOUT_SESSION_CHANGED_EVENT,{detail:{key}}))}
...
export function clearSessionLayoutDraft(key:string){sessionDrafts.delete(key);notifySessionLayoutDraft(key)}
