...
class RegionalZImageAttentionTests(unittest.TestCase):
    ...
    def test_global_only_attention_bias_keeps_entire_image_open(self):
        ...
        document["regions"] = []
        document["prompts"]["background"] = {"positive_source": "", "negative_source": ""}
        positive, _, slots, _ = compile_zimage_attention(document, FakeClip())
        bias = build_joint_attention_bias(slots, positive[0][0].shape[1], 4, 4, 2, 4, 1., torch.device("cpu"), torch.float32, 2)
