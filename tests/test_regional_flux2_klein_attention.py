...
class RegionalFlux2KleinAttentionTests(unittest.TestCase):
        ...
        for mode in ("prompt", "auto", "zero_out"):
            for region_mode in ("empty", "disabled", "detailer"):
                with self.subTest(mode=mode, regions=region_mode):
                    ...
                    clip = FakeClip()
                    positive, negative, slots, _ = compile_flux2_klein_attention(document, clip)
    ...
    def test_global_only_attention_bias_keeps_entire_image_open(self):
        ...
        document["regions"] = []
        document["prompts"]["background"] = {"positive_source": "", "negative_source": ""}
        positive, _, slots, _ = compile_flux2_klein_attention(document, FakeClip())
        bias = build_flux2_joint_attention_bias(slots, positive[0][0].shape[1], 4, 0, 1., 1., torch.device("cpu"), torch.float32, 2)
        padding = positive[0][1]["bv_regional_prefix_padding"]
        self.assertTrue(torch.all(torch.isneginf(bias[..., :padding])))
        self.assertTrue(torch.all(bias[..., padding:] == 0))
